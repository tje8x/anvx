"""Ethereum wallet connector — reads on-chain tx gas costs via public RPC + Etherscan."""
import logging
...
class EthereumWalletConnector:
    ...
    async def validate(self, address: str) -> None:
        ...
        if not address.startswith("0x") or len(address) != 42:
            raise PermissionError("Invalid Ethereum address: must start with 0x and be 42 characters")
        async with httpx.AsyncClient(timeout=15.0) as client:
            resp = await client.post(_RPC, json={"jsonrpc": "2.0", "method": "eth_getBalance", "params": [address, "latest"], "id": 1})
            resp.raise_for_status()
            data = resp.json()
            if "error" in data:
                raise PermissionError(f"Invalid Ethereum address: {data['error'].get('message', 'unknown error')}")
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            for tx in data.get("result", []):
                ...
                records.append(UsageRecord(
                    provider="ethereum_wallet", model="gas", input_tokens=None, output_tokens=None,
    ...
    async def _fetch_txs(client: httpx.AsyncClient, address: str) -> httpx.Response:
        return await client.get(_ETHERSCAN, params={"module": "account", "action": "txlist", "address": address, "startblock": 0, "endblock": 99999999, "sort": "desc", "page": 1, "offset": 1000})
