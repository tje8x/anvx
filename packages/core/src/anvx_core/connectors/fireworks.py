"""Fireworks AI v2 connector — fetches account usage."""
import logging
...
_API = "https://api.fireworks.ai/v1"
...
class FireworksConnector:
    provider = "fireworks"
    kind = "api_key"

    async def validate(self, api_key: str) -> None:
        async with httpx.AsyncClient(timeout=15.0) as client:
            resp = await client.get(f"{_API}/accounts", headers={"Authorization": f"Bearer {api_key}"})
            if resp.status_code == 401:
                raise PermissionError("Invalid Fireworks AI API key")
    ...
    async def fetch_usage(self, api_key: str, since: datetime, until: datetime) -> list[UsageRecord]:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            # Get usage
            resp = await self._fetch_usage(client, headers, account_id)
            resp.raise_for_status()
            data = resp.json()
            ...
            for entry in data.get("data", []):
                ...
                records.append(UsageRecord(
                    provider="fireworks", model=model, input_tokens=input_tokens, output_tokens=output_tokens,
    ...
    async def _fetch_usage(client: httpx.AsyncClient, headers: dict, account_id: str) -> httpx.Response:
        return await client.get(f"{_API}/accounts/{account_id}/usage", headers=headers)
