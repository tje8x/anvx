...
function SpendBar({ current, limit, label }: { current: number; limit: number; label: string }) {
  ...
  return (
    <div className="mb-1">
      <div className="flex justify-between text-[9px] font-data text-anvx-text-dim mb-0.5">
        <span>{label}</span>
        <span>${(current / 100).toFixed(2)} / ${(limit / 100).toFixed(2)} ({pct}%)</span>
      </div>
      <div className="h-1 bg-anvx-bdr rounded-full overflow-hidden"><div className={`h-full ${color} rounded-full`} style={{ width: `${pct}%` }} /></div>
    </div>
  )
}
...
function PolicyCard({ policy, spend, role, onDelete }: { policy: Policy; spend: Spend; role: string; onDelete: () => void }) {
  ...
  return (
    <div className="bg-anvx-bg border border-anvx-bdr rounded p-3 mb-2">
      <div className="flex justify-between items-center mb-1.5">
        <div className="flex items-center gap-2">
          <span className="text-[11px] font-bold font-ui">{policy.name}</span>
          <span className="text-[9px] px-1.5 py-0.5 bg-anvx-win border border-anvx-bdr rounded font-data">{scope}</span>
        </div>
        <div className="flex items-center gap-2">
...
export default function RoutingPage() {
  ...
  const [pDaily, setPDaily] = useState('')
  const [pMonthly, setPMonthly] = useState('')
  const [pPerReq, setPPerReq] = useState('')
  const [pCB, setPCB] = useState('')
  const [pRunway, setPRunway] = useState('')
  const [pAlerts, setPAlerts] = useState<number[]>([80, 90])
  const [pAction, setPAction] = useState('alert_only')
  const [pFailMode, setPFailMode] = useState('open')
  ...
  const [deleteType, setDeleteType] = useState<'rule' | 'policy'>('rule')
  ...
  const fetchAll = useCallback(async () => {
    ...
    const [recsRes, rulesRes, modelsRes, meRes, policiesRes, spendRes] = await Promise.all([
  ...
  const handleSaveRule = async () => {
    ...
    try {
      const h = await authHeaders()
      const res = await fetch(editingRule ? `${API_BASE}/api/v2/routing-rules/${editingRule.id}` : `${API_BASE}/api/v2/routing-rules`, { method: editingRule ? 'PATCH' : 'POST', headers: h, body: JSON.stringify(payload) })
    ...
    } catch (e) { setRuleError(String(e)) } finally { setRuleLoading(false) }
  }
  ...
  const handleSavePolicy = async () => {
    ...
    const payload = {
      name: pName, scope_provider: orNull(pProvider), scope_project_tag: orNull(pProject), scope_user_hint: orNull(pUser),
      daily_limit_cents: orNullNum(pDaily) != null ? Math.round(Number(pDaily) * 100) : null,
      monthly_limit_cents: orNullNum(pMonthly) != null ? Math.round(Number(pMonthly) * 100) : null,
      per_request_limit_cents: orNullNum(pPerReq) != null ? Math.round(Number(pPerReq) * 100) : null,
      circuit_breaker_multiplier: orNullNum(pCB), runway_alert_months: orNullNum(pRunway),
      alert_at_pcts: pAlerts, action: pAction, fail_mode: pFailMode,
    }
    try {
      const h = await authHeaders()
      const res = await fetch(`${API_BASE}/api/v2/policies`, { method: 'POST', headers: h, body: JSON.stringify(payload) })
      if (!res.ok) { const d = await res.json(); setPError(d.detail || 'Failed'); return }
      setPolicyModalOpen(false); await fetchAll(); toast.success('Policy created')
    } catch (e) { setPError(String(e)) } finally { setPLoading(false) }
  }

  const handleDelete = async () => {
    const h = await authHeaders()
    const url = deleteType === 'rule' ? `${API_BASE}/api/v2/routing-rules/${deleteId}` : `${API_BASE}/api/v2/policies/${deleteId}`
    await fetch(url, { method: 'DELETE', headers: h })
    setDeleteOpen(false); await fetchAll(); toast.success('Deleted')
  }
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-6">{MODES.map((m) => (<ModeCard key={m.id} mode={m} selected={mode === m.id} onSelect={() => setMode(m.id)} />))}</div>
      ...
      {rules.length === 0 ? (<p className="text-[11px] font-data text-anvx-text-dim py-4">No rules yet. Create one to let ANVX route within equivalent model groups.</p>) : (
      ...
      <div className="mt-6">
        ...
        {policies.length === 0 ? (<p className="text-[11px] font-data text-anvx-text-dim py-4">No policies yet. Create one to set budget limits and circuit breakers.</p>) : (
      ...
      <Dialog open={policyModalOpen} onOpenChange={setPolicyModalOpen}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          <div className="flex flex-col gap-3 py-2">
            ...
            <div>
              <p className="text-[9px] font-bold font-ui text-anvx-text-dim uppercase mb-1">Alert at</p>
              <div className="flex gap-2">{ALERT_PCTS.map((n) => (<button key={n} onClick={() => toggleAlert(n)} className={`text-[10px] px-2.5 py-1 rounded-full font-data border transition-colors ${pAlerts.includes(n) ? 'bg-anvx-acc border-anvx-acc text-white font-bold' : 'bg-transparent border-anvx-bdr text-anvx-text-dim hover:border-anvx-text'}`}>{n}%</button>))}</div>
            </div>
            <div>
              <p className="text-[9px] font-bold font-ui text-anvx-text-dim uppercase mb-1">Action</p>
              <div className="flex gap-2">{(['alert_only', 'downgrade', 'pause'] as const).map((a) => (<button key={a} onClick={() => setPAction(a)} className={`text-[10px] px-2.5 py-1 rounded-full font-ui border transition-colors ${pAction === a ? 'bg-anvx-acc border-anvx-acc text-white font-bold' : 'bg-transparent border-anvx-bdr text-anvx-text-dim hover:border-anvx-text'}`}>{a}</button>))}</div>
              {pAction === 'downgrade' && rules.filter((r) => r.enabled && r.approved_models.length > 1).length === 0 && (<p className="text-[9px] text-anvx-warn mt-1">No multi-model rules exist. Create one first or this policy will be rejected.</p>)}
            </div>
            <div>
              <p className="text-[9px] font-bold font-ui text-anvx-text-dim uppercase mb-1">Fail mode</p>
              <div className="flex gap-2">{(['open', 'closed'] as const).map((f) => (<button key={f} onClick={() => setPFailMode(f)} className={`text-[10px] px-2.5 py-1 rounded-full font-ui border transition-colors ${pFailMode === f ? 'bg-anvx-acc border-anvx-acc text-white font-bold' : 'bg-transparent border-anvx-bdr text-anvx-text-dim hover:border-anvx-text'}`}>{f}</button>))}</div>
            </div>
      ...
      <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Delete {deleteType === 'rule' ? 'routing rule' : 'spend policy'}?</DialogTitle></DialogHeader>
          <p className="text-[11px] font-ui text-anvx-text-dim py-2">This will be permanently deleted.</p>
          <DialogFooter><DialogClose asChild><MacButton variant="secondary">Cancel</MacButton></DialogClose><MacButton onClick={handleDelete}>Delete</MacButton></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  )
